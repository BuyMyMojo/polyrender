 ...
 * dynamic `import(moduleName)` in @docview/core's `requirePeerDep` into
...
/** Plugin to resolve @docview/core's dynamic peer-dep imports. */
const resolvePeerDeps = {
  name: 'resolve-peer-deps',
  setup(build) {
    build.onLoad({ filter: /docview.*\.(js|ts)$/ }, async (args) => {
      const fs = await import('fs')
      let contents = fs.readFileSync(args.path, 'utf8')

      if (contents.includes('moduleName') && contents.includes('import(')) {
        // Map of peer dep names → static imports
        const peerDeps = [
          'pdfjs-dist', 'epubjs', 'docx-preview',
          'papaparse', 'highlight.js', 'jszip', 'xlsx',
        ]
...
if (existsSync(workerGlob)) {
  const workerFiles = ['pdf.worker.min.mjs', 'pdf.worker.mjs', 'pdf.worker.min.js']
